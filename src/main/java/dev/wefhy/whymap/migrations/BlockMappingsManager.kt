...
class BlockMappingsManager {

    companion object {
        private val md = MessageDigest.getInstance("MD5")
        private val fileWithCurrentVersion = customMappingsDir.resolve("current")

        private val internalMappings: List<BlockMapping.InternalMapping> =
            Companion::class.java.classLoader
                .getResource("blockmappings.txt")
                ?.openStream()
                ?.use { it.readAllBytes() }
        ...
        private val externalMappings: List<BlockMapping.ExternalMapping> = customMappingsDir.listFiles()?.map { file ->
            BlockMapping.ExternalMapping(file)
        } ?: emptyList()

        //TODO this needs to be tested whether internal mappings have priority in the map!
        private val allMappings: MutableMap<String, BlockMapping> = (externalMappings + internalMappings).associateBy { it.hash }.toMutableMap()
    ...
    private val mappings = minecraftBlocks.toList()

    val mappingsJoined = mappings.joinToString("\n")

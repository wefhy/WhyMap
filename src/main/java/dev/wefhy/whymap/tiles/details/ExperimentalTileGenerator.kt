...
class ExperimentalTileGenerator {
    ...
    private fun renderTile(position: ChunkPos): BufferedImage? {
        ...
        val chunk = region.getChunk(position) ?: return null
        val chunkOverlay = region.getChunkOverlay(position) ?: return null
        val biomeFoliage = region.getChunkBiomeFoliageAndWater(position) ?: return null
        val depthmap = region.getChunkDepthmap(position) ?: return null
        val normalmap = region.getChunkNormals(position) ?: return null
